...
When the script exists cleanly or errors, signalize it on the device appropriately via the builtin LED.
 ...
 * provide an user interface for the tool (GUI, TUI or web UI)
   * it lets the user select the image, the script and configure the LED signals
