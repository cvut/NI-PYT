...
See also SSP portal and subscribe there to earn: https://is.fit.cvut.cz/group/ssp/assignee/assignmentdetail?aid=900
...
* Create a tool to synchronize calendar events from google calendar to internal Showmax Phabricator installation via APIs, email or other automated means. Tool should:

a) be documented (basic README, commented source code);

b) have proper logging implemented;

c) be either triggered regularly or run as a daemon.

There is an interface to synchronize Google calendar one way to Phabricator: https://secure.phabricator.com/book/phabricator/article/calendar_imports/. However that doesn’t fit our requirements. For example:

* Many calendars are too big on google so the import ends with “Timeout”.

* We need to be able to filter events somehow.

* Events are not paired by email with Phabricator users.

==== Required outputs

Working prototype in Python that would pass code review and would run on our Jenkins machine in our Debian based docker container (we’ll help with this step) or in our private cloud (also running Debian based docker containers). It can use postgres database if needed. In case it is going to run as a daemon, it should be implemented as 12 factor app.

==== Kontaktní osoby

Showmax, s.r.o.

* adam.cincura@showmax.com[Adam Činčura]
* jan.pipek@showmax.com[Jan Pipek]
